...
class UsersController < ApplicationController
  ...
  def generate_report
    if @current_user.is_teacher?
      ...
      report.text DateTime.now.strftime("Generated by #{@current_user.full_name} on %m/%d/%Y %I:%M%p"), style: :italic, size: 14, align: :center
      ...
      @current_user.taught_course.students.each do |student|
        if student.responses.completed.any?
          ...
          student.responses.completed.each do |response|
            ...
            report.move_down 6
            report.text "Reviewer: " + User.find(response.reviewer_id).full_name
            report.text "Best Sentence Feedback: " + response.best_sentence_feedback
            report.text "Worst Sentence Feedback: " + response.worst_sentence_feedback
            report.move_down 4
      ...
      send_data report.render, type: "application/pdf", target: '_blank'
    else
